...
module.exports = (function() {
  ...
  function getFolder({ type, slugFolderName }) {
    return new Promise(function(resolve, reject) {
      ...
      _getFolderSlugs(mainFolderPath).then(folders => {
        ...
        if (slugFolderName !== undefined && folders.includes(slugFolderName)) {
  ...
  function readMediaList({ medias_list }) {
    return new Promise(function(resolve, reject) {
      ...
      medias_list.forEach(m => {
        if (m.slugFolderName === undefined || m.metaFileName === undefined) {
          return;
        }

        let fmeta = new Promise((resolve, reject) => {
          readMediaAndThumbs(m.slugFolderName, m.metaFileName).then(meta => {
            if (meta === undefined) {
              resolve({});
            }
            meta.metaFileName = m.metaFileName;
            resolve({
      ...
      Promise.all(allMediasData)
        .then(mediasMeta => {
          ...
          mediasMeta.map(d => {
            ...
            folders_and_medias[slugFolderName].medias[metaFileName] = mediaMeta;
            return;
          });
  ...
  function createMedia({ type, slugFolderName, additionalMeta, rawData }) {
    return new Promise(function(resolve, reject) {
      dev.logfunction(
